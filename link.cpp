...
void LinkClass::checkpushblock() {
	...
	if ((t == cPUSH_WAIT || t == cPUSH_HW || t == cPUSH_HW2) && (pushing < 16 || hasMainGuy())) {
		return;
	}
	if ((t == cPUSH_HEAVY || t == cPUSH_HW) && (current_item(itype_bracelet, true) < 1)) {
		return;
	}
	if ((t == cPUSH_HEAVY2 || t == cPUSH_HW2) && (current_item(itype_bracelet, true) < 2)) {
		return;
	}
